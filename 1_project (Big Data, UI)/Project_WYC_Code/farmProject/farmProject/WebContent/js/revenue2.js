...
for (let crop of wyc_area_button) {
  crop.addEventListener("click", () => {
	  ...
	  $.ajax({
	    type: "POST",
	    url: "getAreaCropList.do", // 서버에서 데이터를 가져올 URL (받아올 데이터에 맞게 변경)
...
//	    dataType: "json",
	    success: function(response) {
	    	...
	    	areaKor = revenueArr[0].trim();
	    	
            let str = "";
            for(let i=1; i<revenueArr.length; i=i+2){
            	str += "<div class='" +revenueArr[i].trim()+ " crop_list'>"+
            			"<img src='../img/" +revenueArr[i].trim()+ "-list.jpg'"+
            				"alt='" +revenueArr[i+1].trim()+ "'"+
            				"id='" +revenueArr[i].trim()+ "-img'/>"+
            			"<div class='white-board-" +revenueArr[i].trim()+ " WBC'>"+
            				"<p class='" +revenueArr[i].trim()+ "-text'>" +revenueArr[i+1].trim()+ "</p>"+
            			"</div>"+
            		"</div>";
            }
...
//				    dataType: "json",
		  	  ...
		  	  <div class="card-body row gutter_0">
		  	    <div class="card_crop_img col-lg-4 col-xs-4">
		            <img src="../img/profile_`+revenueArr[0].trim()+`.jpg" style="height: 200px; width: 100%; border-radius: 10px; margin-bottom:10px;">
		          </div>
...
//				    dataType: "json",
				    success: function(response) {
				    	...
				    	drawChartHprice(selectCrop, year, revenueArr);
				    	drawChartYield(selectCrop, year, revenueArr);
...
//농작물 경매단가 차트
function drawChartHprice(selectCrop, year, revenueArr) {
	let hPrice = [];
...
//농작물 생산량 차트
function drawChartYield(selectCrop, year, revenueArr) {
	let yield_arr = [];
