...
import com.farmWYC.command.FarmCostHistoryInsert;
...
import com.farmWYC.command.FarmPolicyInfoCammand;
...
//			viewPage = "revenue1.jsp";
			...
			return;
		}else if(com.equals("/historySaveCost.do")) {
			String wycId = request.getParameter("wycId");
			String selectCrop = request.getParameter("selectCrop");
			String selectCropKor = request.getParameter("selectCropKor");
			String selectArea = request.getParameter("selectArea");
			String costPolicy = request.getParameter("costPolicy");
			String costResult = request.getParameter("costResult");
			...
			request.setAttribute("selectCropKor", selectCropKor);
			request.setAttribute("selectArea", selectArea.trim());
			request.setAttribute("costPolicy", costPolicy);
			request.setAttribute("costResult", costResult);
			
			command = new FarmCostHistoryInsert();
			command.execute(request, response);
			
			viewPage = "serviceCost.jsp";
		}else if(com.equals("/getPolicyInfo.do")) {
			String selectPolicyName = request.getParameter("selectPolicyName");
			
			System.out.println("selectPolicyName : " + selectPolicyName);
			
			request.setAttribute("selectPolicyName", selectPolicyName);
			
			command = new FarmPolicyInfoCammand();
			command.execute(request, response);
			
			response.setContentType("text/plain; charset=utf-8");
			response.getWriter().println(request.getAttribute("selectPolicy"));
			return;
		}
