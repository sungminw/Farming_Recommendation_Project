...
//const jangseong = document.querySelector(".wyc-jangseong");
...
for (let crop of wyc_area_button) {
  crop.addEventListener("click", () => {
	  ...
	  const areaEngDom = document.querySelector(".selectArea");
	  areaEngDom.value = selectArea;
	  
		// 서버에 비동기 요청 보내기 (AJAX) 비용 받아오기
	  $.ajax({
	    type: "GET",
	    url: "areaCropList.do", // 서버에서 데이터를 가져올 URL (받아올 데이터에 맞게 변경)
...
//	    	alert("salesText 연결 성공")
	    	...
	    	areaKor = model[0].areaName;
	    	
            let str = "";
            for(let i=0; i<model.length; i++){
            	str += "<div class='" +model[i].cropEng+ " crop_list'>"+
            			"<img src='/myapp/resources/img/" +model[i].cropEng+ "-list.jpg'"+
            				"alt='" +model[i].cropName+ "'"+
            				"id='" +model[i].cropEng+ "-img'/>"+
            			"<div class='white-board-" +model[i].cropEng+ " crop_list_inner'>"+
            				"<p class='" +model[i].cropEng+ "-text'>" +model[i].cropName+ "</p>"+
            			"</div>"+
            		"</div>";
            }
...
//				    	alert("getCropProfile 연결 성공")
		  	  ...
		  	  <div class="card-body row gutter_0">
		  	    <div class="card_crop_img col-lg-4 col-xs-4">
		            <img src="/myapp/resources/img/profile_`+model[0].cropEng+`.jpg" style="height: 200px; width: 100%; border-radius: 10px; margin-bottom:10px;">
		          </div>
		         <div class="card_crop_list col-lg-8 col-xs-8">
		  	     <ul class="col-xs-12 profile_ul">
		              ...
		              <li class="col-xs-12">특이사항 : `+model[0].cropSpecial+`</li>
			            ...
			            cropProfileList();
//			           	농작물 경매 단가 & 생산량 차트
			            drawChartHpriceYield(selectCrop, model[0].cropName);
...
function cropOverOut() {
	  const crop_lists = document.querySelectorAll(".crop_list");
	  const crop_list_inners = document.querySelectorAll(".crop_list_inner");

	  for (let i = 0; i < crop_lists.length; i++) {
	    const crop_list = crop_lists[i];
	    const crop_list_inner = crop_list_inners[i]; // 해당 인덱스의 WBC 요소와 매칭

	    crop_list_inner.addEventListener("mouseover", () => {
	    	crop_list_inner.style.display = "none";
	    });
	    ...
	    crop_list.addEventListener("mouseout", () => {
	    	crop_list_inner.style.display = "block";
	    });
	  }
	}
...
//농작물 경매 단가 & 생산량 차트
function drawChartHpriceYield(selectCrop, cropName) {
...
//				    	alert("getCropHpriceYield 연결 성공")
				    	...
				    	//경매 단가 및 생산량 모달&차트
				    	drawChartHprice(model, cropName);
				    	drawChartYield(model, cropName);
...
//농작물 경매단가 차트
function drawChartHprice(model, cropName) {
	let hPrice = [];
...
//농작물 생산량 차트
function drawChartYield(model, cropName) {
	let yield_arr = [];
...
//매출 확인 버튼 클릭시 선택된 지역 및 농작물 값을 가져와서 매출 예측
...
//농작물과 지역 둘 다 체크 되어있어야 매출 분석 진행
modalButton.addEventListener("click", (e) => {
	if(areaEng === null || cropEng === null){
...
//		    	alert("salesText 연결 성공")
		    	...
		    	const salesResult = model[model.length - 1].toLocaleString();
...
//비용 확인 버튼 클릭스 필요 데이터 체크 확인
const submit_cost = document.querySelector(".submit_cost");
//const selectCropKor = document.querySelector(".selectCropKor");

submit_cost.addEventListener("click", (e) => {
	const salesResult = document.querySelector(".salesResult");
